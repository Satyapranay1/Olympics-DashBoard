...
def tally(df):
    tal = df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'])
    tal = tal.groupby('NOC').sum(numeric_only=True)[['Gold', 'Silver', 'Bronze']].sort_values('Gold', ascending=False).reset_index()
    tal['total'] = tal['Gold'] + tal['Silver'] + tal['Bronze']
    tal[['Gold', 'Silver', 'Bronze', 'total']] = tal[['Gold', 'Silver', 'Bronze', 'total']].astype(int)
    return tal
...
def most_successful(df, sport):
    ...
    return x[['Name', 'Count', 'Sport', 'region']]
